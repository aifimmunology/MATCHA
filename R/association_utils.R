...
#' @param outputDF A boolean, deteremines whether to out put a dataframe or SummarizedExperiment with results. 
...
associatedPredictions <- function(SE1 = NULL, SE2, assay1, assay2, sampleColumn, interModel, firstPair, secondPair, adjust = TRUE, outputDF =FALSE){
    ...
    }else if(model_type == 'scRNA_Associations' & se1_type != 'scRNA'){
    ...
    }else if(model_type == 'General_Associations' & c(!se2_type %in% c('General','Transformed', 'ChromVAR') | 
                                                      !se1_type %in% c('General', 'Transformed', 'Absent'))){
    ...
    if(se1_type != 'Absent'){
        ...
        if(all(!SE1@colData[,sampleColumn] %in% SE2@colData[,sampleColumn]) |
            ...
            if(se1_type == 'scATAC'){
                ...
                SE1 <- MOCHA::subsetMOCHAObject(SE1, subsetBy = sampleColumn, 
    ...
    pair = as.list(paste(firstPair, secondPair, sep ='__'))
    ...
    ## Log transform the FragmentCountss & CellCounts so as to stabilize the model. But only if FragmentCounts is in the model. Same for CellCounts.
    if(any(colnames(metaData ) %in% c('FragmentCounts'))){
        metaData$rawFragmentCounts = metaData$FragmentCounts
        metaData$FragmentCounts <- log10(metaData$FragmentCounts)
    }
